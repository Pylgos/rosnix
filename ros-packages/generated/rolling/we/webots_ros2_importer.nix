{
  ament_copyright,
  buildRosPackage,
  builtin_interfaces,
  fetchgit,
  fetchurl,
  fetchzip,
  rosSystemPackages,
  substituteSource,
  xacro,
}:
let
  sources = rec {
    webots_ros2_importer-01b3ce07bc8aebfa3c652bdce6277e0a8aa8a453 = substituteSource {
      src = fetchgit {
        name = "webots_ros2_importer-01b3ce07bc8aebfa3c652bdce6277e0a8aa8a453-source";
        url = "https://github.com/ros2-gbp/webots_ros2-release.git";
        rev = "01b3ce07bc8aebfa3c652bdce6277e0a8aa8a453";
        hash = "sha256-0BjkWBfT9UijOpEXMxJlQSl5ecFhKxV3II4hqj3MLsY=";
      };
      substitutions = [
      ];
    };
  };
in
buildRosPackage {
  pname = "webots_ros2_importer";
  version = "2023.1.3-1";
  src = sources.webots_ros2_importer-01b3ce07bc8aebfa3c652bdce6277e0a8aa8a453;
  nativeBuildInputs = [  ] ++ rosSystemPackages.getPackages { forNativeBuildInputs = [  ]; };
  propagatedNativeBuildInputs = [  ] ++ rosSystemPackages.getPackages { forNativeBuildInputs = [ "python3-lark-parser" ]; };
  buildInputs = [  ] ++ rosSystemPackages.getPackages { forBuildInputs = [  ]; };
  propagatedBuildInputs = [ builtin_interfaces xacro ] ++ rosSystemPackages.getPackages { forBuildInputs = [ "python3-collada" ]; };
  depsTargetTarget = [  ] ++ rosSystemPackages.getPackages { forDepsTargetTarget = [  ]; };
  depsTargetTargetPropagated = [  ] ++ rosSystemPackages.getPackages { forDepsTargetTarget = [  ]; };
  checkInputs = [ ament_copyright ] ++ rosSystemPackages.getPackages { forBuildInputs = [ "python3-numpy" "python3-pil" "python3-pycodestyle" "python3-pytest" ]; };
  meta = {
    description = "This package allows to convert URDF and XACRO files into Webots PROTO files.";
  };
}
