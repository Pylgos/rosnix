  ...
  sources = rec {
    ...
    mrpt_libhwdrivers = substituteSource {
      src = fetchgit {
        ...
        rev = "5de223f74631df11b68eadf421009931a300fadf";
        hash = "sha256-m+gHxKL5DqkLIx8upsrmbef1+ZexpEKt8C7f7OeAi4g=";
      };
      substitutions = [
        {
          path = "CMakeLists.txt";
...
buildRosPackage {
  ...
  buildInputs = [  ];
  propagatedBuildInputs = [ assimp cv_bridge ffmpeg freeglut freenect glfw3 libGL libGLU libjpeg libpcap libusb1 mrpt_libgui mrpt_libmaps mrpt_libslam opencv python3Packages.pip rclcpp rosbag2_storage tinyxml-2 udev wxGTK32 xorg.libXrandr zlib ];
  depsTargetTarget = [  ];
